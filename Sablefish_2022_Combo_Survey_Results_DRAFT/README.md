...
Other metadata looked at, but not included due to poor performance for this particular FCNN model, were sex, month, and days into the year. The same model, except without latitude, has an R-squared of 0.9526 (APE = ???). Putting back latitude and only removing length gives an R-squared of 0.9652 (APE = 7.718). The adding back length and removing weight has an R-square = ??? (APE = ???). The spectra only model (no metadata) has an R squared of 0.9378 (APE = 8.788), with a predicted N of 1,556 since there is no missing metadata.  
