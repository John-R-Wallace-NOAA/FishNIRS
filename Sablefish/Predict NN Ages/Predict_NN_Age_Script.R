...
if(interactive()) {
   setwd("C:/ALL_USR/JRW/SIDT/Sablefish/Predict NN Ages")  # Change path as needed
} else { 
    options(width = 140)
} 
...
# --- Save() ages and write out to a CSV file - Example of file name using Date() function: 'New Ages for 2019 Hake, 26 Sep 2023.csv'
...
load("C:\\ALL_USR\\JRW\\SIDT\\Sablefish\\Predict NN Ages\\Predicted_Ages\\NN Predicted Ages, 11 Oct 2023.RData")
load("C:\\ALL_USR\\JRW\\SIDT\\Sablefish\\Keras_CNN_Models\\Sable_2017_2019 21 Nov 2022.RData")
Delta <- -0.2
Predicted_Ages_Path <- "Predicted_Ages"
...
New_Ages$Age_Rounded <- round(New_Ages$NN_Pred_Median + Delta)
New_Ages$Rounded_Age <- factor(" ")
New_Ages <- JRWToolBox::match.f(New_Ages, Sable_2017_2019, 'filenames', 'filenames', 'TMA')
