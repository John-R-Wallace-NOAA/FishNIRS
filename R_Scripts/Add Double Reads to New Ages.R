...
load("C:\\SIDT\\CLPR_Combo_1985__2024\\CLPR_Combo_1985__2024_Str_Wgt_Sex\\Predicted_Ages\\NN Predicted Ages, 19 Mar 2025.RData")
headTail(New_Ages)
...
sum(!is.na(New_Ages$specimen_id))
...
browsePlot('print(xyplot(NN_Pred_Median ~ TMA | factor(Year), group = factor(Reader), data = New_Ages_Double,  col = c("dodgerblue", "red", "green", "goldenrod3", "cyan"), pch = c(1, 0, 2, 1, 4), 
            ...
            file = "Figur/NN_Pred_vs_TMA_by_Reader_and_Year.png")
...
browsePlot('print(xyplot(NN_Pred_Median ~ TMA | factor(Year), group = factor(Reader), data = New_Ages_Double_Rnd,  col = c("dodgerblue", "goldenrod3", "green", "red", "cyan"), pch = c(1, 0, 4, 1, 2, 3), 
            ylab = paste0("NN Pred Median (decimal; not bias corrected) or TMA Secondary Double Read"), xlab = "TMA Best Age (circles) or TMA Primary Read (x, square, triangle)",
            panel = function(...) {panel.xyplot.loess(..., span = 1.0); panel.abline(0, 1) }, as.table = TRUE,
            key = list(space = "right", points = list(col=c("dodgerblue", "red", "green", "goldenrod3",  "cyan"), pch = c(1, 1, 4, 0, 2)), 
            text = list(c("Spectra vs Don", "Spectra vs Tyler", "Tyler vs Don", "Patrick vs Don", "Patrick vs Tyler")))))', 
            file = "Predicted_Ages/NN_Pred_Rnd_vs_TMA_by_Reader_and_Year.png")
