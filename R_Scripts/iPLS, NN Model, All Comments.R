...
print(headTail(Model_Spectra.sg.iPLS, 3, 0, 3, 5))
...
Rdm_reps <- 20 # j loop
num_folds <- 20 # i loop    # Checking how many folds work best for metadata only model
   ...
   for (i in 1:num_folds) {
       ...
       while(file.exists('Run_NN_Model_Flag')) {    # The multiple full fold version breaks the stop by removing the file flag, but it remains for now
		  ...
		  if(Iter > 1) {
              ...
              Time_Left_Min <- ((Rdm_reps - j + 1) * num_folds * Iter_Num + (num_folds - i + 1) * Iter_Num + (Iter_Num - Iter + 1)) * Iter_Loop_Time_Min
...
for(numRdmModels in 1:Rdm_reps) {
   ...
   Stats_RDM_median_by_model <- rbind(Stats_RDM_median_by_model, data.frame(t(unlist(Correlation_R_squared_RMSE_MAE_SAD(TMA_Vector, round(y.fold.test.pred_RDM_median_one_rep + Delta))))))
}
