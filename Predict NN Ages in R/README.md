...
- After scanning new material comparable to what the NN model was trained on (e.g. otloliths, or gonad material, from the same fish species), put the Bruker spectra files from OPUS into the 'New_Scans' folder. (e.g. the filename(s) will look like: PACIFIC_HAKE_BMS201906206C_1191_OD1.0). For testing, there are 'old' spectra files used to train the Hake FCNN model currently in 'New_scans'.  (Do not put anything else in the 'New_Scans' folder except spectra files.)
