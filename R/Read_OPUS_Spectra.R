...
# ------------------------------------ Main User Setup ------------------------------------------------------------
	...
	# (5) Sablefish 2019, Combo survey
    if(Spectra_Set == "Sable_Combo_2019") { 
        if(is.null(Spectra_Path)) 
           Spectra_Path <- 'C:/ALL_USR/JRW/SIDT/Sablefish 2019 Combo/Sable_2021_Scans'
        if(Meta_Add) {
		    is.null(Meta_Path)
...
# --- Create a character vector of all spectral files within 'Spectra_Path'---   
    ...
    if(Meta_Add) {
	    ...
	    Model_Spectra_Meta <- match.f(Model_Spectra_Meta, metadata_DW, "specimen_id", "AgeStr_id", c('Length_cm', 'Weight_kg', 'Month_Scaled', 'Depth_m', 'Sex', 'Days_into_Year')) 
        ...
        if(!is.null(Model_Spectra_Meta$Days_into_Year_prop_max))
           Model_Spectra_Meta$Days_into_Year <- (Model_Spectra_Meta$Days_into_Year - min(Model_Spectra_Meta$Days_into_Year, na.rm = TRUE))/(max(Model_Spectra_Meta$Days_into_Year, na.rm = TRUE) - min(Model_Spectra_Meta$Days_into_Year, na.rm = TRUE))
        ...
        # TF <- Model_Spectra_Meta$percent_crystallized_scan <= 15 & Model_Spectra_Meta$percent_crystallized_scan <= 10 & !is.na(Model_Spectra_Meta$Month_Scaled) & 
        ...
        TF <- Model_Spectra_Meta$percent_crystallized_scan <= 15 & Model_Spectra_Meta$percent_crystallized_scan <= 10 & is.na(Model_Spectra_Meta$tissue_level_scan) & 
		          !is.na(Model_Spectra_Meta$Length_cm) & !is.na(Model_Spectra_Meta$structure_weight_g) & !is.na(Model_Spectra_Meta$Month_Scaled)
